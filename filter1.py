...
input_csv_file_path = 'csv/gender_chn.csv'
output_csv_file_path = 'csv/output.csv'
filter = ''
temp = ''
...
if __name__ == "__main__":
    ...
    csv_to_json(output_csv_file_path, 'json/femaleNewborn.js')
    
    filter = 'Age population, age 00, male, interpolated'
    temp = 'maleNewborn'
    filtered_data = filter_data(data, my_filter)
    write_array_to_csv(filtered_data, output_csv_file_path)
    csv_to_json(output_csv_file_path, 'json/maleNewborn.js')

    filter = 'Fertility rate, total (births per woman)'
    temp = 'fertility'
    filtered_data = filter_data(data, my_filter)
    write_array_to_csv(filtered_data, output_csv_file_path)
    csv_to_json(output_csv_file_path, 'json/fertility.js')
    
    filter = 'Mortality rate, adult, male (per 1,000 male adults)'
    temp = 'maleMortality'
    filtered_data = filter_data(data, my_filter)
    write_array_to_csv(filtered_data, output_csv_file_path)
    csv_to_json(output_csv_file_path, 'json/maleMortality.js')

    filter = 'Mortality rate, adult, female (per 1,000 female adults)'
    temp = 'femaleMortality'
    filtered_data = filter_data(data, my_filter)
    write_array_to_csv(filtered_data, output_csv_file_path)
    csv_to_json(output_csv_file_path, 'json/femaleMortality.js')

    filter = 'Life expectancy at birth, male (years)'
    temp = 'femaleMortality'
    filtered_data = filter_data(data, my_filter)
    write_array_to_csv(filtered_data, output_csv_file_path)
    csv_to_json(output_csv_file_path, 'json/maleLifeExpectancy.js')

    filter = 'Life expectancy at birth, female (years)'
    temp = 'femaleMortality'
    filtered_data = filter_data(data, my_filter)
    write_array_to_csv(filtered_data, output_csv_file_path)
    csv_to_json(output_csv_file_path, 'json/femaleLifeExpectancy.js')
